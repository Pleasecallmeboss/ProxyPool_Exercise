...
def convert_proxy_or_proxies_withscore(data):
    """
    convert list of str to valid proxies or proxy
    :param data:
    :return:
    ...
    if isinstance(data, list):
        ...
        for item in data:
            # skip invalid item
            item2 = item[0].strip()
            if not is_valid_proxy(item2): continue
            host, port = item2.split(':')
            result.append(Proxy(host=host, port=int(port),score=int(item[1])))
        return result
    if isinstance(data, tuple):
        host, port = data[0].split(':')
        return Proxy(host=host, port=int(port),score=int(data[1])) 
